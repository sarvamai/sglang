...
Every six hours, the workflow checks stable releases of `sgl-project/sglang`.
It mirrors exact upstream commits and opens draft PRs into Sarvam `main`.

- `sglang/release-vX.Y.Z` records an immutable release and is its PR's source.
...
- Already-integrated releases need no PR; open or merged release PRs prevent
  duplicates, including squash/rebase merges. Closing an
...
2. Store a fine-grained PAT as **`REPOSITORY_SYNC_TOKEN`**, scoped to the target
...
3. From a trusted checkout, with that credential supplied as `GH_TOKEN`, run:

   ```bash
   python3 .github/sarvam/sync_releases.py --repo sarvamai/sglang --disable-upstream-workflows
   python3 .github/sarvam/sync_releases.py --repo sarvamai/sglang --dry-run
   ```
...
4. After merging, manually run **Sarvam sync upstream releases** on `main`, first
   with `dry_run: true`, then with `dry_run: false`. Verify the mirrors and PRs.
   The six-hour schedule is active after merge. Manual runs on other branches skip.
