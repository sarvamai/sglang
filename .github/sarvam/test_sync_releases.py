...
class ReleaseTests(unittest.TestCase):
    ...
    def test_unregistered_or_active_workflows_block_sync(self):
        ...
        for state in (None, "active", "disabled_inactivity", "disabled_fork"):
            workflows = [] if state is None else [{"path": path, "state": state}]
            with self.assertRaisesRegex(RuntimeError, "registered and disabled"):
                sync.check_workflows([path], workflows)
        sync.check_workflows([path], [{"path": path, "state": "disabled_manually"}])

    def test_disable_keeps_our_workflow_enabled(self):
        workflows = [{"path": sync.WORKFLOW, "state": "active", "id": 1}]
        workflows += [
            {"path": ".github/workflows/upstream.yml", "state": "active", "id": 2}
        ]
        with (
...
class GitSyncTests(unittest.TestCase):
    def setUp(self):
        ...
        self.prs = []
        self.workflows = [{"path": sync.WORKFLOW, "state": "active"}]
                ...
                self.prs.append(
                    {
                        ...
                        "head": {
                            ...
                            "repo": {"full_name": "sarvamai/sglang"},
                        },
                    }
                )
